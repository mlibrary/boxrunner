...
RSpec.describe PackageFindingAidJob, type: :job do
  ...
  before do
    # The um_arclight gem is not yet available (see TODOs in the job), so stub
    # the constants the job depends on.
  ...
  it 'queues the job on the index queue' do
    expect { described_class.perform_later(identifier, 'html') }
      .to have_enqueued_job(described_class).with(identifier, 'html').on_queue('index')
  end

  context 'when the format is html' do
    it 'generates html and enqueues an html.success event' do
      expect { described_class.perform_now(identifier, 'html') }.not_to raise_error
  ...
  context 'when the format is pdf' do
    it 'generates pdf and enqueues a pdf.success event' do
      expect { described_class.perform_now(identifier, 'pdf') }.not_to raise_error
  ...
  context 'when the format is unsupported' do
    it 'raises a GenerateError without generating or enqueuing' do
      expect { described_class.perform_now(identifier, 'txt') }
  ...
  context 'when generation fails' do
    ...
    it 'enqueues a failure event and re-raises as a GenerateError' do
      expect { described_class.perform_now(identifier, 'html') }
        .to raise_error(UmArclight::GenerateError, identifier)
